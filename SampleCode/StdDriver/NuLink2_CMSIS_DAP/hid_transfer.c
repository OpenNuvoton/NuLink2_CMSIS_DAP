...
static uint8_t g_u8OutBuf[DAP_PACKET_SIZE];
static uint8_t g_u8InBuf[DAP_PACKET_SIZE];
...
void USBD20_IRQHandler(void)
{
    ...
    if (IrqStL & HSUSBD_GINTSTS_EPBIF_Msk)
    {
        ...
        if (HSUSBD->EP[EPB].EPINTSTS & HSUSBD_EPINTSTS_BUFFULLIF_Msk)
            EPB_Handler();
...
void EPB_Handler(void)  /* Interrupt OUT handler */
{
    ...
    DAP_ExecuteCommand(g_u8OutBuf, g_u8InBuf);
